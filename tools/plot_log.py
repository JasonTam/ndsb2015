...
def get_log_parsing_script():
    ...
    dirname = '~/documents/caffe/tools/extra'
...
def load_data(data_file, field_idx0, field_idx1):
    ...
    with open(data_file, 'r') as f:
        for line in f:
            ...
            if line[0] != '#':
                fields = line.split()
                if len(fields) > max(field_idx0, field_idx1):
...
def plot_chart(chart_type, path_to_png, path_to_log_list):
    ...
    elif hasattr(chart_type, '__iter__'):
        chart_types = chart_type
...
def is_valid_chart_type(chart_type):
    check = lambda n: n >= 0 and n < len(get_supported_chart_types())
    if type(chart_type) == int:
        return check(chart_type)
    elif hasattr(chart_type, '__iter__'):
        return all(map(check, chart_type))
...
if __name__ == '__main__':
    if len(sys.argv) < 4:
        ...
        chart_type = [int(ii) for ii in sys.argv[1].split(',')]
